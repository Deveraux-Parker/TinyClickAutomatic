...
import pyautogui
import time  # Optional: For additional control if needed
...
class BrowserWindow(QMainWindow):
    def __init__(self):
        ...
        self.processor = AutoProcessor.from_pretrained("Samsung/TinyClick", trust_remote_code=True)
        self.model = AutoModelForCausalLM.from_pretrained("Samsung/TinyClick", trust_remote_code=True).to(self.device)
        self.setup_ui()
        self.setup_browser()
    ...
    def analyze_and_move(self):
        ...
        generated_texts = self.processor.batch_decode(outputs, skip_special_tokens=False)
        ...
        if 'click_point' in result:
            ...
            pyautogui.moveTo(screen_x, screen_y, duration=1, tween=pyautogui.easeInOutQuad)
